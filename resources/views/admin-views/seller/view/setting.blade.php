    ...
    <div class="content container-fluid">
        ...
        <div class="row">
            ...
            <div class="col-md-6 mt-2">
                <div class="card">
                    <div class="card-header">
                        <h5 class="mb-0">{{\App\CPU\translate('District')}}</h5>
                    </div>

                    <div class="card-body">
                        <form action="{{ url()->current() }}" method="GET">
                            @csrf
                            <label for="exampleInputdistrict" class="title-color d-flex gap-1 align-items-center">{{\App\CPU\translate('District')}}</label>
                            <select class="form-control form-control-user" id="exampleInputdistrict" name="district_id" required>
                                <option value="" disabled selected></option>
                                @foreach($districts as $district)
                                <option value="{{$district->id}}" {{$seller['district_id'] == $district->id ? 'selected':''}}>{{$district->name}}</option>
                                @endforeach
                            </select>
                            <div class="d-flex justify-content-end pt-3">
                                <button type="submit" class="btn btn--primary">{{\App\CPU\translate('Save')}}</button>
