            ...
            <div class="container px-10px">
                <div class="collapse navbar-collapse" id="navbarCollapse"
                    ...
                    <ul class="navbar-nav" style="{{Session::get('direction') === "rtl" ? 'padding-right: 0px' : ''}}">
                        ...
                        <li class="nav-item dropdown">
                            <a class="nav-link dropdown-toggle" href="#" data-toggle="dropdown">{{ \App\CPU\translate('Smart Village') }}</a>

                            <ul class="dropdown-menu __dropdown-menu-sizing dropdown-menu-{{Session::get('direction') === "rtl" ? 'right' : 'left'}} scroll-bar" style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};">
                                <?php
                                    $districtz = \App\Model\District::with('sellers')->orderBy('name', 'ASC')->get();
                                ?>
                                @foreach($districtz as $districtzs)
                                    @if($districtzs->sellers->count() > 0)
                                        <li class="__inline-17">
                                            <div>
                                                <a class="dropdown-item" href="{{route('sellers')}}?district_id={{$districtzs->id}}">
                                                    {{$districtzs->name}}
                                                </a>
                                            </div>
                                        </li>
